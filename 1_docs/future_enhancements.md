...
### Phase 1: ルールのデータベース化とインフラの最適化
   - 現在コード内に書かれている判定ロジックをテーブル化し、管理画面から「感度」を調整可能にする。
   - **ポーリング方式の検討**: 現在は GitHub Actions (5分間隔) でのポーリングをメインとしているが、本番環境構築時には、より高頻度かつ安定した実行が可能な Vercel Cron (1分間隔〜) への完全移行を再検討する。
...
## 4. 運用固まり後に実装予定（サブUI・個人登録まわり）

運用が固まってから検討する、個人登録・サブ通知・Slack連携の要件です。

### 4.1 個人用の地点登録と「管理者向けのみ通知」

- **概要**: プライベート（自宅・実家など）で被災した場合、全社には通知せず、**管理者にだけ何らかの通知**がいくようにする。
- **想定**: 個人用の地点登録で「この地点は全社通知対象にしない／管理者のみ通知」のような設定を可能にする。

### 4.2 Slack が使えないときのサブ通知

- **概要**: Slack に送信できない、または一定時間たっても回答が来ない場合を想定した**サブ通知機能**を搭載する。
- **想定**: あらゆる情報通信手段へ発信できるようにする（メール、SMS、アプリプッシュ、電話など、手段は要検討）。

### 4.3 Slack から完結する操作

- **概要**: 上記 4.1 や 4.2 に関わる設定・操作も、**ページ（ポータル）を開かなくても Slack から行える**ようにする。
- **想定**: 地点登録の有無や「管理者のみ通知」のオンオフ、サブ通知先の確認・変更などが Slack 上でできるとよい。
